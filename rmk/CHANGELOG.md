...
- Add configurable Sticky Keys for modifiers, layers, and repeated modified keys, with named profiles, timeout/repeat limits, configurable release triggers (including modifier or layer key-up after an elapsed timeout), and `OSM`/`OSL` compatibility aliases
