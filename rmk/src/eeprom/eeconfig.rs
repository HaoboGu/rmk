...
//     pub(crate) async fn init_with_default_config(&mut self) {
//         self.set_enable(true).await;
//         self.set_default_layer(0).await;
//         self.set_keymap_config(EeKeymapConfig::default()).await;
//         self.set_backlight_config(EeBacklightConfig::default())
//             .await;
//         self.set_audio_config(EeAudioConfig::default()).await;
//         self.set_rgb_light_config(EeRgbLightConfig::default()).await;
//         self.set_layout_option(0).await;
//     }

//     /// Initialize eeprom with given eeconfig
//     pub(crate) async fn init_with_config(&mut self, config: Eeconfig) {
//         self.set_enable(config.eeprom_enable).await;
//         self.set_default_layer(config.default_layer).await;
//         self.set_keymap_config(config.keymap_config).await;
//         self.set_backlight_config(config.backlight_config).await;
//         self.set_audio_config(config.audio_config).await;
//         self.set_rgb_light_config(config.rgb_light_config).await;
//         self.set_layout_option(config.layout_option).await;
//     }

//     /// Enable or disable eeprom by writing magic value
//     pub(crate) async fn set_enable(&mut self, enabled: bool) {
...
//         self.write_byte(0, &buf).await;
//     }

//     /// Returns eeprom magic value stored in EEPROM
//     pub(crate) async fn get_magic(&mut self) -> u16 {
...
//     /// Set default layer
//     pub(crate) async fn set_default_layer(&mut self, default_layer: u8) {
...
//     /// Set keymap config
//     pub(crate) async fn set_keymap_config(&mut self, config: EeKeymapConfig) {
...
//             }
//         };
//         self.write_byte(KEYMAP_CONFIG_ADDR, &buf).await;
//     }
...
//     pub(crate) async fn set_backlight_config(&mut self, config: EeBacklightConfig) {
//         let buf = match config.pack() {
//             Ok(b) => b,
//             Err(_) => {
...
//             }
//         };
//         self.write_byte(BACKLIGHT_CONFIG_ADDR, &buf).await;
//     }
...
//     /// Set audio config
//     pub(crate) async fn set_audio_config(&mut self, config: EeAudioConfig) {
//         let buf = match config.pack() {
//             Ok(b) => b,
//             Err(_) => {
...
//             }
//         };
//         self.write_byte(AUDIO_CONFIG_ADDR, &buf).await;
//     }
...
//     pub(crate) async fn set_rgb_light_config(&mut self, config: EeRgbLightConfig) {
//         let buf = match config.pack() {
//             Ok(b) => b,
//             Err(_) => {
...
//             }
//         };
//         self.write_byte(RGB_CONFIG_ADDR, &buf).await;
//     }
...
//     /// Set layout option
//     pub(crate) async fn set_layout_option(&mut self, option: u32) {
//         let mut buf = [0xFF; 4];
//         BigEndian::write_u32(&mut buf, option);
//         self.write_byte(LAYOUT_OPTION_ADDR, &buf).await;
//     }
