        ...
        if let Some(key_type) = StorageKeys::from_u8(buffer[0]) {
            match key_type {
                ...
                StorageKeys::TapDanceData => {
                    ...
                    let mut actions = [Action::No; 4];
                    for i in 0..4 {
                        let key_action = from_via_keycode(BigEndian::read_u16(&buffer[3 + i * 2..5 + i * 2]));
                        actions[i] = key_action.to_action();
                    }

                    let tap_dance = TapDance::new_from_vial(actions[0], actions[2], actions[3], actions[1], timeout);
