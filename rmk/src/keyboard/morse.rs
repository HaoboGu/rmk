    ...
    Keyboard<'a, ROW, COL, NUM_LAYER, NUM_ENCODER>
{
    ...
    pub(crate) async fn handle_morse_timeout(&mut self, key: &HeldKey, morse: Morse<MAX_MORSE_PATTERNS_PER_KEY>) {
        ...
        if self
            ...
            .keys
            .iter()
            .filter(|k| k.morse.is_some() && matches!(k.state, KeyState::Held(_))) ///FIXME: is this correct? or KeyState::Held(MorsePattern::default()) needed here?
            .count()
            > 0
        {
            return;
                ...
                let action = match k.state {
                    KeyState::Held(pattern) => {
                        ...
                        if k.timeout_time < Instant::now() {
                            ...
                            if pattern.is_full() || pattern.pattern_length() >= morse.actions.max_pattern_length() { //FIXME: is this correct?
