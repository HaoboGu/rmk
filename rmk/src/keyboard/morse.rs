    ...
    pub(crate) async fn handle_morse_timeout(&mut self, key: &HeldKey, morse: Morse<MAX_MORSE_PATTERNS_PER_KEY>) {
        match key.state {
            KeyState::Held(pattern) => {
                let a = morse.action_from_pattern(pattern.followed_by_hold()); //FIXME: is this correct?
            ...
            KeyState::IdleAfterTap(pattern) => {
                let a = morse.action_from_pattern(pattern); //FIXME: is this correct? //long idle, means the end of the morse pattern
        ...
        if self
            ...
            .keys
            .iter()
            .filter(|k| k.morse.is_some() && matches!(k.state, KeyState::Held(_)))
            .count()
            > 0
        {
            return;
            ...
            match self.held_buffer.find_pos_mut(event.pos) {
                Some(k) => {
                    ...
                    if let KeyState::IdleAfterTap(pattern) = k.state {
                        if pattern.is_full() || pattern.pattern_length() >= morse.actions.max_pattern_length() {
                            //FIXME: is this correct?
                            // Reach maximum tapping number
                            k.state = KeyState::Held(pattern);
                        } else {
                            k.state = KeyState::Held(pattern.followed_by_tap()); //FIXME: is this correct?
                        }
                ...
                let action = match k.state {
                    KeyState::Held(pattern) => {
                        ...
                        if k.timeout_time < Instant::now() {
                            ...
                            Some(morse.action_from_pattern(pattern)) //FIXME: is this correct? or followed_by missing?
                        } else {
                            // Not timeout, check whether it's the last tap action
                            if pattern.is_full() || pattern.pattern_length() >= morse.actions.max_pattern_length() {
                                //FIXME: is this correct?
                                // It's the last tap action, trigger the tap action immediately
                                let action = morse.action_from_pattern(pattern); //FIXME: is this correct?
                                ...
                                k.state = KeyState::IdleAfterTap(pattern);
                    ...
                    KeyState::PostHold(pattern) => {
                        ...
                        Some(morse.action_from_pattern(pattern)) //FIXME: is this correct?
                    }
