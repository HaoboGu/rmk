    ...
    Keyboard<'a, ROW, COL, NUM_LAYER, NUM_ENCODER>
{
    ...
    pub(crate) fn should_release_osm_early(&self, action: &Action) -> bool {
        if self.osm_state.value().is_none() {
            return false;
        }

        let release_keys = &self.keymap.borrow().behavior.one_shot_modifiers.release_modifier_keys;

        let key_action = KeyAction::Single(*action);
        release_keys.contains(&key_action)
    }

    pub(crate) async fn release_osm_early(&mut self) {
        debug!("Releasing one-shot modifiers early");
