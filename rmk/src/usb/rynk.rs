...
pub fn build_host_usb<D: Driver<'static>>(builder: &mut Builder<'static, D>) -> (HostUsbReader<D>, HostUsbWriter<D>) {
    ...
    static RX_BUF: StaticCell<[u8; RYNK_USB_MAX_PACKET_SIZE as usize]> = StaticCell::new();

    let state = STATE.init(State::new());
    let class = CdcAcmClass::new(builder, state, RYNK_USB_MAX_PACKET_SIZE);
    let (sender, receiver) = class.split();
    let receiver = receiver.into_buffered(RX_BUF.init([0; RYNK_USB_MAX_PACKET_SIZE as usize]));
    (receiver, sender)
}
...
/// Rynk USB reader: skips zero-length reads.
/// CDC-ACM has no in-band EOF; disconnect surfaces as `Err`.
struct RynkUsbRx<'a, D: Driver<'static>> {
    receiver: &'a mut BufferedReceiver<'static, D>,
}

impl<D: Driver<'static>> ErrorType for RynkUsbRx<'_, D> {
    type Error = <BufferedReceiver<'static, D> as ErrorType>::Error;
}

impl<D: Driver<'static>> Read for RynkUsbRx<'_, D> {
    async fn read(&mut self, buf: &mut [u8]) -> Result<usize, Self::Error> {
        loop {
            let n = self.receiver.read(buf).await?;
            if n > 0 || buf.is_empty() {
                return Ok(n);
            }
        }
