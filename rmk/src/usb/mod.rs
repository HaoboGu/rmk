...
pub(crate) fn new_usb_builder<'d, D: Driver<'d>>(driver: D, keyboard_config: DeviceConfig<'d>) -> Builder<'d, D> {
    ...
    // Extra interfaces (usb_log, steno, rynk, dfu) overflow the 128-byte config descriptor buffer.
    #[cfg(any(feature = "usb_log", feature = "steno", feature = "rynk", feature = "dfu"))]
    const USB_BUF_SIZE: usize = 256;
    #[cfg(not(any(feature = "usb_log", feature = "steno", feature = "rynk", feature = "dfu")))]
