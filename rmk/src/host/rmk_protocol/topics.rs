...
//!
//! BLE-only topics (`BatteryStatus`, `BleStatusChange`) are gated behind
//! `cfg(feature = "_ble")`. The `_ble` build subscribes to those in addition
//! to the base topics. USB-only builds skip them.
...
/// Run the topic publisher loop forever. One instance per active transport.
///
...
pub async fn run_topic_publisher<Tx: WireTx + Clone>(sender: Sender<Tx>) -> ! {
    let mut seq: u32 = 0;
    let mut next_seq = || {
        let s = seq;
    ...
    loop {
        // Two select layers because embassy-futures' `select` family caps at
        // four arms; we add an extra layer when BLE subscribes to two more
        // topics.
        let base = async {
            select4(
