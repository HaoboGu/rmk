...
impl<'a> Keyboard<'a> {
    ...
    pub async fn process_inner(&mut self, event: KeyboardEvent) {
        ...
        // Capture the press time once per event and thread it through.
        ...
        if self.combo_on {
            if let (Some(key_action), is_combo) = self.process_combo(key_action, event, press_time).await {
                self.process_key_action(&key_action, event, is_combo, press_time).await
            }
        } else {
            self.process_key_action(key_action, event, false, press_time).await
        }
    }
    ...
    async fn process_key_action(
        ...
        match updated_decision_for_cur_key {
            KeyBehaviorDecision::CleanBuffer | KeyBehaviorDecision::Release => {
                ...
                self.process_key_action_inner(key_action, event, press_time).await
            }
    ...
    async fn process_key_action_inner(
        ...
        if !key_action.is_morse() {
            ...
            self.process_key_action_morse(&key_action, event, press_time).await;
        }
            ...
            && max_size > 0
        {
            ...
            self.held_buffer.push(HeldKey::new(
                ...
                *key_action,
                KeyState::WaitingCombo,
                press_time,
                press_time + self.keymap.combo_timeout(),
            ));
