...
//         let current_magic = eeprom.get_magic().await;
//         // if current_magic != 0 {
...
//                 .unwrap();
//             match eeconfig {
//                 Some(eeprom_config) => eeprom.init_with_config(eeprom_config).await,
//                 None => eeprom.init_with_default_config().await,
//             }
//             eeprom.set_keymap(keymap).await;
//         }
...
//     pub(crate) async fn write_byte(&mut self, mut address: u16, data: &[u8]) {
//         if data.is_empty() {
...
//             if self.check_consolidation().await {
...
//     async fn write_record(&mut self, record: EepromRecord) {
//         let mut buf = [0xFF; 1];
...
//     async fn read_record(&mut self, address: u16) -> Option<EepromRecord> {
//         let mut bytes = [0u8; 4];
...
//     async fn check_consolidation(&mut self) -> bool {
...
//             self.consolidate_records().await;
...
//     async fn consolidate_records(&mut self) {
