...
/// Get the current BLE profile index.
pub(crate) fn get_current_ble_profile() -> u8 {
    BLE_STATUS.lock(|c| c.get().profile)
}

/// Update only the BLE connection state, preserving the current profile.
pub(crate) fn set_ble_connection_state(state: BleState) {
    set_ble_status(BleStatus {
        profile: get_current_ble_profile(),
        state,
    });
}
...
>(
    ...
    join(background_task, async {
        loop {
            ...
            set_ble_connection_state(BleState::Advertising);
                    ...
                    ConnectionType::Usb => {
                        ...
                        .await
                        {
                            Either4::First(_) => {
                                ...
                                set_ble_connection_state(BleState::Inactive);
...
async fn gatt_events_task(server: &Server<'_>, conn: &GattConnection<'_, '_, DefaultPacketPool>) -> Result<(), Error> {
    ...
    loop {
        ...
        if connected && !published_connected_state {
            set_ble_connection_state(BleState::Connected);
...
#[cfg(test)]
mod tests {
    use super::{BLE_STATUS, BleState, BleStatus, set_ble_connection_state, set_ble_status};
    ...
    #[test]
    fn set_ble_connection_state_preserves_current_profile() {
