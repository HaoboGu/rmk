...
mod one_shot_test {
    ...
    rusty_fork_test! {
        ...
        /// OSM Test Case 14 - Early Release
        ///
        /// Config:
        /// - timeout: 10000ms (very long)
        /// - activate_on_keypress: true
        /// - send_on_second_press: false
        /// - release_modifier_keys: [OneShotLayer(1)]
        ///
        /// Sequence:
        /// - Press and Release OSM LShift
        /// - Press OSL Layer 1 (should release OSM early)
        /// - Release OSL Layer 1
        /// - Press and Release A
        ///
        /// Expected:
        /// - LShift is sent first
        /// - LShift is released when OSL is pressed
        /// - A is sent without LShift (from layer 1)
        /// - All released
        #[test]
        fn test_osm_early_release_with_layer_switch() {
            ...
            key_sequence_test! {
                    ...
                    BehaviorConfig {
                        one_shot_modifiers: OneShotModifiersConfig {
                            ...
                            release_modifier_keys: {
                                ...
                                let _ = v.push(rmk::types::action::KeyAction::Single(Action::OneShotLayer(1)));
                                v
                            },
                            ..OneShotModifiersConfig::default()
                        },
                ...
                sequence: [
                    [0, 0, true, 10],   // Press OSM LShift
                    [0, 0, false, 10],  // Release OSM LShift
                    [0, 1, true, 10],   // Press OSL Layer 1 (should release OSM early)
                    [0, 1, false, 10],  // Release OSL Layer 1
                    [0, 2, true, 10],   // Press key at (0,2), should get C from layer 1 WITHOUT shift
                    [0, 2, false, 10],  // Release key
                ],
                expected_reports: [
                    [KC_LSHIFT, [0, 0, 0, 0, 0, 0]], // LShift is sent from the start
                    [0, [0, 0, 0, 0, 0, 0]],         // LShift is released when OSL is pressed
                    [0, [kc_to_u8!(C), 0, 0, 0, 0, 0]], // C from layer 1 WITHOUT LShift
                    [0, [0, 0, 0, 0, 0, 0]],         // All released
                ]
            };
        }

        /// OSM Test Case 15 - Early Release with Multiple Modifiers
        ///
        /// Config:
        /// - timeout: 10000ms (very long)
        /// - activate_on_keypress: true
        ...
        /// - Press and Release OSM LShift
        /// - Press and Release OSM LCtrl
        /// - Press OSL Layer 1 (should release both OSM early)
        /// - Release OSL Layer 1
        /// - Press and Release A
        ///
        /// Expected:
        /// - LShift is sent first
        /// - LShift + LCtrl is sent
        /// - Both modifiers are released when OSL is pressed
        /// - A is sent without modifiers (from layer 1)
        /// - All released
        #[test]
        fn test_osm_early_release_multiple_modifiers() {
            use rmk::types::action::Action;
            ...
            key_sequence_test! {
                ...
                sequence: [
                    ...
                    [0, 4, true, 10],   // Press OSM LCtrl
                    [0, 4, false, 10],  // Release OSM LCtrl
                    [0, 1, true, 10],   // Press OSL Layer 1 (should release both OSM early)
                    [0, 1, false, 10],  // Release OSL Layer 1
                    [0, 2, true, 10],   // Press key at (0,2), should get C from layer 1 WITHOUT modifiers
                    [0, 2, false, 10],  // Release key
                ],
                expected_reports: [
                    [KC_LSHIFT, [0, 0, 0, 0, 0, 0]],        // LShift is sent from the start
                    [KC_LSHIFT | KC_LCTRL, [0, 0, 0, 0, 0, 0]], // LCtrl is added to combination
                    [0, [0, 0, 0, 0, 0, 0]],                // Both modifiers are released when OSL is pressed
                    [0, [kc_to_u8!(C), 0, 0, 0, 0, 0]],     // C from layer 1 WITHOUT modifiers
                    [0, [0, 0, 0, 0, 0, 0]],                // All released
                ]
            };
        }
