...
# The simulator harness itself (tests/common/simulator) is the implementation of that
# API, so it's allowed to touch the raw primitives; only scenarios are checked.
if rg -n -e "$forbidden" "$repo_root/rmk/tests" -g '*.rs' -g '!**/common/simulator.rs' -g '!**/common/simulator/**'; then
