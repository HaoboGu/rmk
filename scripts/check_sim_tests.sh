...
forbidden="$forbidden|TestKeyboard|get_keymap\(|morse_behavior_config|get_combos_config"
...
if rg -n -e '(^| )pub const [A-Z0-9_]*KEYMAP:|(^| )const [A-Z0-9_]*KEYMAP:' "$repo_root/rmk/tests" -g '*.rs' \
    | rg -v 'rmk/tests/common/mod.rs:.*TEST_KEYMAP|rmk/tests/common/morse.rs:.*MORSE_KEYMAP'; then
