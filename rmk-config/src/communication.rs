...
impl KeyboardTomlConfig {
    pub fn get_communication_config(&self) -> ConfigResult<CommunicationConfig> {
        let usb_enabled = self
            .keyboard
            .clone()
            .unwrap_or_default()
            .usb_enable
            ...
            (None, Some(c)) => {
                if !c.enabled {
                    Err(ConfigError::Validation {
                        field: "ble/usb".to_string(),
                        message: "You must enable at least one of usb or ble".to_string(),
                    })
                } else {
