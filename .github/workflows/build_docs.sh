...
# Usage: build_docs.sh <book-lang> <dest-dir>
#
# Build the course as of the date specified specified in the
# POT-Creation-Date header of docs/po/$book_lang.po. The output can be
# found in $dest_dir.
#
...
if [ "$book_lang" = "en" ]; then
    ...
    pot_creation_date=$(grep --max-count 1 '^"POT-Creation-Date:' "docs/po/$book_lang.po" | sed -E 's/".*: (.*)\\n"/\1/')
    ...
    export MDBOOK_OUTPUT__HTML__SITE_URL=/rmk/$book_lang/
