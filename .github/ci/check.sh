...
emit_batch_args() {
    ...
    printf -- '--- %s --manifest-path rmk-types/Cargo.toml --features rmk_protocol\n' "$cmd"
}
...
cargo +stable clippy --target-dir "$clippy_target" --manifest-path rmk-types/Cargo.toml --features rmk_protocol -- -D warnings
