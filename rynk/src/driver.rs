...
/// Frees the slot on drop, so a cancelled request releases its slot and its
/// late reply is dropped as unmatched instead of reaching the slot's next
/// user.
struct SlotGuard<'a> {
    client: &'a Client,
    idx: usize,
...
pub struct Client {
    ...
    /// In-flight requests; the driver delivers each reply here by SEQ. A
    /// reply no slot waits for (from a cancelled request or a
    /// fire-and-forget command) is dropped.
...
impl Client {
    ...
    ///
    /// Waits until a topic arrives. If the link dies this never finishes on
    /// its own — the surrounding `select` (or a watch on the driver task)
    /// must cancel it.
    ...
    ///
    /// This crate has no async runtime, so there is no built-in timeout: a
    /// silent peer keeps this pending forever, and if the link dies it never
    /// finishes on its own — the surrounding `select` (or a watch on the
    /// driver task) must cancel it, just like [`next_topic`](Self::next_topic).
    /// Callers that need a deadline wrap it in their runtime's timeout.
    ...
    pub(crate) async fn request<E: Endpoint>(&self, req: &E::Request) -> Result<E::Response, RynkHostError> {
        ...
        // A decode that leaves trailing bytes means the firmware sent a
        // different type than we expect; reject it instead of silently
        // accepting a prefix.
    ...
    async fn send_frame<Req: Serialize>(&self, cmd: Cmd, seq: u8, req: &Req) -> Result<(), RynkHostError> {
        ...
        let mut buf: FrameBytes = {
            ...
            // a request too big for the buffer fails at `encode_frame` below
            // instead.
...
impl<R: Read, W: Write> Driver<R, W> {
    ...
    pub async fn run(&mut self, client: &Client) -> RynkHostError {
                ...
                while let Some(len) = df.next(buf) {
                    ...
                    if header.cmd.is_topic() {
                        ...
                        if let Err(TrySendError::Full(event)) = client.topics.try_send(event) {
                            // Drop the oldest topic to make room: topics are
                            // best-effort, and the read loop must never block.
        ...
        let tx_loop = async {
            // Send a lone `0x00` first. If stale bytes sit in the peer's
            // receive buffer (an OS port probe, a half-frame from a prior
            // session), the zero ends them as garbage instead of letting them
            // merge with our first request.
