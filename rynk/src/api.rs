...
use rmk_types::protocol::rynk::endpoint::Topic;
...
/// Decode one topic payload, typed by the shared topic table.
fn topic<T: Topic>(payload: &[u8]) -> Option<T::Payload> {
    postcard::take_from_bytes::<T::Payload>(payload).ok().map(|(v, _)| v)
}
...
impl<T: Read + Write> Client<T> {
    /// Read the next topic push, decoded into a typed [`Event`] — an
    /// unrecognized topic or a payload that fails to decode falls back to
    /// [`Event::Unknown`]. Queued topics are returned first. Cancel-safe.
    ///
    ...
    pub async fn next_event(&mut self) -> Result<Event, TransportError> {
        ...
        let decoded = match frame.cmd {
            Cmd::LayerChange => topic::<command::LayerChange>(&frame.payload).map(Event::LayerChange),
            Cmd::WpmUpdate => topic::<command::WpmUpdate>(&frame.payload).map(Event::WpmUpdate),
            Cmd::ConnectionChange => topic::<command::ConnectionChange>(&frame.payload).map(Event::ConnectionChange),
            Cmd::SleepState => topic::<command::SleepState>(&frame.payload).map(Event::SleepState),
            Cmd::LedIndicatorChange => topic::<command::LedIndicatorChange>(&frame.payload).map(Event::LedIndicator),
            Cmd::BatteryStatusChange => topic::<command::BatteryStatusChange>(&frame.payload).map(Event::BatteryStatus),
            _ => None,
        };
        Ok(decoded.unwrap_or(Event::Unknown(frame)))
    }
