...
1. BLE peripheral doesn't work
   ...
   In the new version, RMK uses nRF chip's unique address as the device address. If BLE peripheral doesn't work after updating, it's likely that the old fixed device address is stored in the chip. You should [clear the storage](/guide/features/storage) for both central and peripheral:
