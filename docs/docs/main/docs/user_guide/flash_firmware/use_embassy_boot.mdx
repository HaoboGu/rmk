...
`dfu-util` will automatically detect the board when it is in DFU mode. The RMK firmware enters DFU mode on reset, or (optional) when the `unlock_keys` configured in `keyboard.toml` are pressed.
...
1. Press the key combination configured as `unlock_keys` in `keyboard.toml` (default: `[[0, 0], [1, 1]]`)
2. If an LED is configured, it will light up for half a second, then blink **"D F U"** in Morse code (`-.. ..-. ..-` — dash-dot-dot, dot-dot-dash-dot, dot-dot-dash)
3. You now have **10 seconds** to start the DFU download
4. Once the download begins, the LED stays on — the firmware update is in progress
5. After the update completes, the LED turns off and the board reboots

If no DFU download starts within 10 seconds, the lock re-engages and the LED turns off. Press the unlock keys again to retry.
