...
A GPIO pin that lights up during DFU transfer. On the Raspberry Pi Pico, the built-in LED is on pin 25.

#### `unlock_keys` (optional, dfu_lock)

When the `dfu_lock` feature is enabled, the keyboard starts with DFU **locked**. To unlock, all keys in `unlock_keys` must be pressed simultaneously. The keys are identified by their physical matrix position `(row, column)`, not by keycode.
