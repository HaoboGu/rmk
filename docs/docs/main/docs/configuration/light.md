...
`pin` field is just like IO pins in [`[matrix]`](./keyboard_matrix.md#matrix-configuration), and `low_active` defines whether the light is low-active or high-active (`true` means low-active).
