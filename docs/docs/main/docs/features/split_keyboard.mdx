...
::: info There are many examples for split keyboards in [RMK examples folder](https://github.com/rmk-rs/rmk/tree/main/examples/use_config). :::
...
RMK supports BLE wireless split on nRF52, nRF54, ESP32 and Pi Pico W right now. For BLE split, the central and peripheral parts are connected via BLE, and the host is connected to the central via USB or BLE.
