...
The header is parsed out of place by `RynkHeader::parse(&[u8; 5])`, giving
`cmd`, `seq`, and `payload_len`. Outbound frames are written into a
caller-owned buffer by the `RynkMessage` wrapper (`RynkMessage::build` for
requests/responses, `build_topic` for pushes), which lays down the header
and postcard-encodes the payload in one pass. Both live in
[`rmk-types/src/protocol/rynk/message.rs`](../rmk-types/src/protocol/rynk/message.rs)
and are shared between firmware and the `rynk` host client.
...
See `rmk-types/src/protocol/rynk/command.rs` for the canonical enum — the
`Cmd` constants and topic table are generated from the `endpoints!` /
`topics!` declarations there. Each group is sequential under its hex
prefix:
...
| `ConnectionChange` | `0x8003` | `ConnectionStatus` | `ConnectionChangeEvent` | - |
| `SleepState` | `0x8004` | `bool` | `SleepStateEvent` | - |
| `LedIndicatorChange` | `0x8005` | `LedIndicator` | `LedIndicatorEvent` | - |
| `BatteryStatusChange` | `0x8006` | `BatteryStatus` | `BatteryStatusEvent` | `_ble` |

Dispatch uses `is_topic()` (`cmd as u16 & 0x8000 != 0`).

Each non-persistent topic (`WpmUpdate`, `SleepState`, `LedIndicatorChange`)
...
as a discrete slice up to MTU − 3 (244 B for typical 247 B MTU). The
receiver reassembles exactly as above — read a chunk, append, and split
off complete `5 + LEN` frames:

```rust
loop {
    let chunk = link.read_chunk().await; // one GATT notification
...
Every wire-payload type derives `postcard::MaxSize`. `RYNK_MAX_PAYLOAD`
(in [`command.rs`](../rmk-types/src/protocol/rynk/command.rs)) folds the
maximum request, response, and topic size over the union of enabled
features at compile time; `RYNK_MIN_BUFFER_SIZE`
(in [`message.rs`](../rmk-types/src/protocol/rynk/message.rs)) adds the
5-byte header. Adding a command can never under-size the buffer.
